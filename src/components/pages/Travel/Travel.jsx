...
function Travel(props) {
    const TABLE_HEADS = ['No.', 'Id', 'Tour Name', 'Start Day', 'Start Location', 'Price', 'Remain'];
    ...
    const fetchTravels = async () => {
        ...
        if (response && response.data && response.data.EC === '0') {
            ...
            setListTravelSearch(response.data.DT.travels);
        }
    ...
    useEffect(() => {
        if (searchCategory === 'Name') {
            const filteredTravels = listTravel.filter((item) => {
                // Lọc dựa trên tên của travel và từ khóa tìm kiếm
                const travelName = item['Tour']['tourName'].toLowerCase(); // Đảm bảo không phân biệt hoa thường
                const searchLower = searchText.toLowerCase(); // Đảm bảo không phân biệt hoa thường
                return travelName.includes(searchLower);
            });
            setListTravelSearch(filteredTravels);
        }
        else if (searchCategory === 'Id') {
            const filteredTravels = listTravel.filter((item) => {
                // Lọc dựa trên id của travel và từ khóa tìm kiếm
                const travelId = String(item['id']); // Đảm bảo không phân biệt hoa thường
                return travelId.includes(searchText);
            });
            setListTravelSearch(filteredTravels);
        }
                ...
                <div className="flex flex-1">
                    ...
                    <div className="relative w-72">
                        <input
                            ...
                            type="search"
                            id="search-dropdown" className="block p-2.5 pl-4 w-full h-full z-20 text-sm focus:outline-none focus:border-blue-600 border border-blue-gray-100 placeholder-blue-300 text-blue-800 font-medium rounded-e-lg focus:ring-1" placeholder="Search ..." required />
